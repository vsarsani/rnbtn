...
#' 
#' This function aggregrates model results from rnbtn_model_pergene function for all genes in a parallel fashion
#' 
...
#' @param fctrel : A list of column names and desired factor relevels .Example :  rlist<- list(condition = c("CONTROL","T1","T2",),strain = c("WT","KO","DO"),slevel = c("none","LOW","MEDIUM","HIGH"),
#'batch = c("1","2","3","4"))
...
#' @cores the number of cores to run in parallel
#' @ctype : socket type for parallel operation. "PSOCK" runs both on windows and linux. "FORK" runs only on linux but is much faster

#' 
#' @examples 
#' 
#formula <- as.formula(exp~strain/condition/slevel+batch+log(N))
#fctrel <- list(condition = c("CONTROL","DIS1","DIS2"),strain = c("WT","KP","DO"),slevel = c("none","LOW","MEDIUM","HIGH"),
#batch = c("1","2","3","4"))
#rnbtn_model_agg_parallel(df,formula,locus_tag="gene_name",fctrel=fctrel,iter=5,a=0,cores=10,ctype="PSCOCK")
...
rnbtn_model_agg_parallel <- function(df,formula,locus_tag=locus_tag,fctrel=NONE,iter=5,a=0,cores=10,ctype="PSCOCK"){ 
  ...
  ## validate input df
    ...
    stop("Input data frame is not in the data frame data type. Please convert  and re-run it")
  }
    if(ncol(df)< 3){
    stop("The data frame should have atleast one covariate, one response and locus/gene name column")
  }
if(!is.null(fctrel)){
...
#### Modeling Function
rnbtn_model_pergene <- function(df,formula,res_name,locus_tag=locus_tag,iter=iter,a=a) {
             ...
             ## Model starts here 
              model = tryCatch(
                ...
                for (i in 1:iter)
                   {
                  fit <- cv.glmnet(x, y, alpha = a, lambda = lambda_seq,family = negative.binomial(theta = t),parallel=FALSE,intercept = FALSE)
...
for (i in names(fctrel)) {
  ...
  return(suppressWarnings(suppressMessages(rnbtn_model_pergene(df_g,formula,res_name,locus_tag="g",iter=5,a=0))))
  gc()
}
...
no_cores <- cores
cl <- makeCluster(no_cores, type=ctype)
registerDoParallel(cl)
cat("Running model in parallel.Running with",no_cores,"cores and",ctype,"\n")
    
    
# do parallel function to apply rnbtn_model_pergene in parallel
locusresults <- foreach(i=1:length(locuslist)) %dopar% genewisefits(df,i,locuslist,fctrel)
    

## Post-Process and storing results in a data frame
#Here L1 is dummy header given by melt function
suppressWarnings(mod_data <- melt(locusresults)%>%dplyr::select(-L1))
...
suppressWarnings(mod_data  <- mod_data%>%mutate(coeff_log2value=as.numeric(coeff)/log(2))%>%filter(!effect %in% (1:col_length)))

# Restore genes/locus_tags that failed model as "NA"  so that we have an record of them
#missedresults<- list()
