...
#' the change in transposon insertions attributable to
#' gene-environment changes
#without transformations or uniform normalization
#'
#' This function aggregrates model results from rnbtn_model_pergene
#'  function for all genes in a parallel fashion
#'
...
#' error associated with each lambda value, and then choose lambda.Default is 10
# Note: iter increases your run time
#' @cores the number of cores to run in parallel
#' @ctype : socket type for parallel operation. "PSOCK" runs both on
...
#'batch = c("1","2","3","4"))
#'rnbtn_model_agg_parallel(df,formula,locus_tag="gene_name",
#'fctrel=fctrel,iter=5,a=0,cores=10,ctype="PSCOCK")
...
locusresults <- foreach(i = 1 : length(locuslist)) %dopar%
...
suppressWarnings(suppressMessages(
  ...
  if (nrow(mod_data %>% filter(locus_tag == g)) == 0) {
    ...
    missedresults[[g]] <- missed %>% mutate(locus_tag = g, effect = value,
...
suppressWarnings(suppressMessages(
  missed_data <- melt(missedresults) %>% dplyr::select(-L1)))
