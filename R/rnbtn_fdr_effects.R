...
#' change in transposon insertions attributable to gene-environment changes
#without transformations or uniform normalization
#'
#' This function constructs locfdr value for each mean effect
#'
#' @param df : dataframe of model results from
#' rnbtn_model_agg.R or rnbtn_model_agg_parallel.R
...
#' @param fecutoff : A cutoff for seperating frankly essential genes.
#' Default is one. This will filter out genes/locus tags with
#' < user defined cutoff
#' @param cecutoff: A cutoff for conditionally essential genes
#' to be excluded from
#' fdr calculations.Default is one. This will apply filter
#' : pmax(mean, controlmean) > cutoff and pmin(mean, controlmean) > cutoff
#' @param path: A path where you want frankly essential and conditionally
#' essential files written: ex:
#' path <- "../../reports/fdr_csv/" in windows
...
#'rnbtn_fdr_effects(df,dfmean,exeffects=exeffects,
#'fecutoff=1,cecutoff=1,path,method=locfdr)
...
rnbtn_fdr_effects <- function(df, dfmean, locus_tag = "locus_tag",
                              exeffects = c("Intercept", "batch", "log"),
                              fecutoff = 1, cecutoff = 1,
                              path, method ="locfdr") {
  ...
  ## validate input df
    ...
    stop("Input Model data frame is not in the
         data frame data type. Please convert and re-run it")
  }
    if (ncol(df) < 3) {
    stop("The data frame should have locus_tag,
         effect and coefficient")
  }
if (is.data.frame(dfmean) == FALSE) {
    stop("Input Means data frame is not in the data
         frame data type. Please convert and re-run it")
  }

    if (ncol(df) < 4) {
    stop("The Means data frame should have locus_tag,
         effect,mean and controlmean")
  }
...
# Create two empty dataframes, one for frankly
#essential and another one for conditionally essential
...
coef.df <- data.frame(locus_tag = locus, effect = i,
                      value = vl) %>% filter(value != "NA")
...
coef.df <- coef.df %>%
  ...
  mutate(n_log2_coefficient =
    (log2_coefficient - mean(log2_coefficient)) / sd(log2_coefficient))
...
coef.df <- coef.df %>%
 ...
 coef.df <- coef.df %>%
  ...
  if (is.null(coef.df)) {
    ...
    cat("Unable to fdr for effect ", i,
        "Please change fdr method or input df", "\n")
...
suppressWarnings(suppressMessages(
  for (g in locuslist) {
     if (nrow(coef.df %>%
       ...
       missedresults[[g]] <- missed %>% mutate(locus_tag = g,
                                               effect = value,
                                               log2_coefficient = "NA") %>%
         dplyr::select(-value)
    }
...
suppressWarnings(suppressMessages(
  missed.df <- melt(missedresults) %>% dplyr::select(-L1)))
missed.df <- left_join(missed.df, count.df,
                       by = c("locus_tag", "effect")) %>%  mutate(fdr = "NA")
...
#frankly essential
suppressMessages(lapply(1:length(fe_df),
                        function(i) write.csv(fe_df[[i]],
                       file.path(path, paste0(str_replace_all(names(fe_df[i]),
                        ":", "_"), "_essential.csv")), row.names = FALSE)))
# conditionally essential

suppressMessages(lapply(1:length(ce_df),
                       function(i) write.csv(ce_df[[i]],
                        file.path(path, paste0(str_replace_all(names(ce_df[i]),
                        ":", "_"), "_effect.csv")), row.names = FALSE)))
}
