...
#' estimate the change in transposon insertions
#' attributable to  gene-environment changes
#' without transformations or uniform normalization
#'
#' This function aggregrates model results from
#' rnbtn_model_pergene function for all genes in a serial fashion
#'
...
#' @param iter: Number of times to run cross validation to take the
#' mean error associated with each lambda value, and then
#' choose lambda.Default is 10
...
rnbtn_model_agg <- function(df, formula, locus_tag=locus_tag,
  ...
  ## validate input df
    ...
    stop("Input data frame is not in the data frame data
         type.Please convert  and re-run it")
  }
    if (ncol(df) < 3) {
    stop("The data frame should have atleast one
         covariate,one response and locus/gene name column")
  }
if (!is.null(fctrel)) {
...
rnbtn_model_pergene <- function(df, formula, res_name,
             ...
             ## Model starts here
              model <- tryCatch({
                ...
                lambdas <-  NULL
                for (i in 1:iter) {
                  fit <- cv.glmnet(x, y, alpha = a, lambda = lambda_seq,
            ...
            best_ridge <- glmnet(x, y, alpha = a, lambda = best_lambda,
                                family = negative.binomial(theta = t),
                                intercept = FALSE)
                },
                error = function(err) {
             ...
             ## Writing row of NA's if model is null.
             #Usually Coefficient Output has 8 columns. Change this
             if (is.null(coef(model))) {
                 ...
                 coeffs <- t(as.matrix(vals[, -(which(colSums(vals) == 0))]))
                  ...
                  cat("model complete for this locus_tag :",
                      locus_tag, "\n")
                 return(coeffs)
                 }
            }
...
# Restore genes/locus_tags that failed model as
# "NA"  so that we have an record of them
missedresults <- list()
...
suppressWarnings(suppressMessages(
  missed_data <- melt(missedresults) %>%
    dplyr::select(-L1)))
