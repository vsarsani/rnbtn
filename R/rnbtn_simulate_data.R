...
#'
#' This function simulates  transposon count data for Total counts
#' and Unique Counts
#'
#' @param

#' n_strain= Number of strains/genetic backgrounds or
#' different organisms to simulate
#' n_condition= Number of experimental or environmental conditions to simulate
#' n_slevel= Number of stress levels to simulate for each condition
#' n_rep = Number of replicates for each stress level

#'
#' @examples
#'
#' rnbtn_simulate_data(n_strain=2,n_condition=2,
#' n_slevel=2,n_rep=2)
...
rnbtn_simulate_data <- function(n_strain, n_condition, n_slevel, n_rep) {
    ...
    condition <- rep(1:n_condition, times = n_slevel,
                     each = 4000 * n_rep * n_strain)
    strain <- rep(1:n_strain,
                  each = 4000 * n_slevel * n_condition * n_rep)
    cov <- data.frame(strain, slevel, condition, Rep)
    cov_sel <- sapply(colnames(cov), function(name) {
        paste(name, cov[, name], sep = "_")
    })
    locus_tag <- rep(seq.int(4000),
                     n_rep * n_slevel * n_condition * n_strain)
