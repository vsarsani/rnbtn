...
#'
# TC_files <-
# list.files('../../reports/TC_fdr_csv',pattern='*effect.csv',full.names =
# TRUE) UC_files <-
# list.files('../../reports/UC_fdr_csv',pattern='*effect.csv',full.names =
# TRUE) path <- '../../reports/fdr_csv/'
...
rnbtn_intersect_effects <- function(TC_files, UC_files, path, fdrcutoff = 0.05) {
    ...
    for (i in seq_len(TC_files)) {
        tryCatch({
            ...
            dfplot1 <- df %>%
                mutate(fdr = case_when(Unique.fdr < fdrcutoff ~ "Unique_sig"))
            # seperate out total significant
            dfplot2 <- df %>%
                mutate(fdr = case_when(Total.fdr < fdrcutoff ~ "Total_sig"))
            # seperate out non significant
            dfplot3 <- df %>%
                mutate(fdr = case_when(Total.fdr > fdrcutoff & Unique.fdr > fdrcutoff ~
                  "ns"))
            # combine
            ...
            # plot data frame
            dfplot <- dfplot %>%
                mutate(pointsize = case_when(fdr == "ns" | fdr == "ns" ~ "1", fdr ==
                  "Total_sig" | fdr == "Unique_sig" ~ "1"))
            # ggplot
            suppressWarnings(p <- ggplot(dfplot, aes(Total.coefficient, Unique.coefficient)) +
            ...
            write_csv(dfplot2 %>%
                filter(fdr == "Total_sig"), (file.path(path, paste0("TC_", condition,
                "_sig.csv"))[1]))
            write_csv(dfplot1 %>%
                filter(fdr == "Unique_sig"), (file.path(path, paste0("UC_", condition,
                "_sig.csv"))[1]))
            dfb <- arrange(df %>%
                filter(Unique.fdr < fdrcutoff & Total.fdr < fdrcutoff), Total.fdr,
                Unique.fdr)
            write_csv(dfb, (file.path(path, paste0("TC_", condition, "_sig.csv"))[1]))
            # write plots
            suppressWarnings(ggsave(filename = (file.path(path, paste0(condition,
