...
if(all(c('fdr','log2_coefficient','locus_tag','mean','controlmean') %in% names(tdf))==TRUE)
    {
    tdf <- tdf%>%dplyr::select(fdr,log2_coefficient,locus_tag,mean,controlmean)
}
else {
...
if(all(c('fdr','log2_coefficient','locus_tag','mean','controlmean') %in% names(udf))==TRUE)
    {
    udf <- udf%>%dplyr::select(fdr,log2_coefficient,locus_tag,mean,controlmean)
}
else {
...
#ggplot
suppressWarnings(p <- ggplot(dfplot,aes(Total.coefficient,Unique.coefficient))+ geom_point(aes(shape=fdr, color=fdr,size=pointsize))+xlab("Total Counts Coefficient")+ylab("Unique Counts Coefficient")+theme_bw()+theme(axis.line = element_line(colour = "black"),
...
theme(aspect.ratio=1)+theme(panel.border = element_blank(),axis.line    = element_line(color='black')))
# Write csv files of total sig,unique sig and both to the paths
write_csv(dfplot2%>%filter(fdr=="Total_sig"),(file.path(path,paste0("TC_",condition, "_sig.csv"))[1]))
write_csv(dfplot1%>%filter(fdr=="Unique_sig"),(file.path(path,paste0("UC_",condition, "_sig.csv"))[1]))
dfb<- arrange(df%>%filter(Unique.fdr  < fdrcutoff & Total.fdr < fdrcutoff),Total.fdr,Unique.fdr)
